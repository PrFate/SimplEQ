...
juce::AudioProcessorValueTreeState::ParameterLayout SimplEQAudioProcessor::createParameterLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterChoice>("LowCut Slope", "LowCut Slope", slopes, 0));
    layout.add(std::make_unique<juce::AudioParameterChoice>("HighCut Slope", "HighCut Slope", slopes, 0));
    return layout;
}
